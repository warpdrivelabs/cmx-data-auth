...
| `backend/.dockerignore` | 构建上下文白名单（上下文=backend/ 目录，只送本仓 + cmx-rulesengine、cmx-container） |
...
```bash
docker run -d --name cmx-dataauth -p 8098:8098 \
...
```bash
cd backend/cmx-data-auth/docker
mkdir -p runtime/config
docker compose run --rm cmx-dataauth cat /opt/cmx/config-template.toml > runtime/config/docker.toml
